...
<main>
	...
	<section class="px-4 mx-auto lg:px-6 max-w-7xl">
		<div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
			{#if albums}
				{#each albums.filter((album) => album.fields && album.fields.Artwork && album.fields.Artwork.length > 0 && album.fields['Album Title'] && album.fields.Band) as album}
					<div class="relative flex flex-col gap-2 card card-hover variant-filled-primary">
						<a href={album?.fields['Music Link']} target="_blank" rel="">
							<img
								src={album?.fields?.Artwork && album?.fields.Artwork.length > 0
									? album?.fields.Artwork[0].url
									: 'default.jpg'}
